...
class SettingDlg(wx.Dialog):
    def __init__(self):
        ...
        wx.Dialog.__init__(self, parent = None, id = wx.ID_ANY, title = "设置")
        ...
        self._stockCountSpinCtrl = wx.SpinCtrl(self._pageGeneral, id = wx.ID_ANY, value = "3", pos = pos, size = (64, 16), max = 0xFFFF)
        pos = [20, pos[1] + self._stockCountStaticText.GetSize().GetHeight() + CTRL_CY]
        self._calmColorSettingCtrl = colorSettingCtrl.ColorSettingCtrl(self._pageGeneral, "持平色：", color = wx.WHITE, pos = pos, size = COLORSETTINGCTRL_SIZE)
        pos[1] += CTRL_CY + COLORSETTINGCTRL_SIZE[1]
        self._gainColorSettingCtrl = colorSettingCtrl.ColorSettingCtrl(self._pageGeneral, "升值色：", color = wx.RED, pos = pos, size = COLORSETTINGCTRL_SIZE)
        pos[1] += CTRL_CY + COLORSETTINGCTRL_SIZE[1]
        self._payColorSettingCtrl = colorSettingCtrl.ColorSettingCtrl(self._pageGeneral, "贬值色：", color = wx.GREEN, pos = pos, size = COLORSETTINGCTRL_SIZE)
        pos[1] += CTRL_CY + COLORSETTINGCTRL_SIZE[1]
        self._backColorSettingCtrl = colorSettingCtrl.ColorSettingCtrl(self._pageGeneral, "背景色：", color = wx.BLACK, pos = pos, size = COLORSETTINGCTRL_SIZE)
        #股票页。
