...
def parseBigCode(text):
    bigCode = stock.Stock()
    bigCode._datas[stock.Stock.NAME][0] = "大盘"
    ...
    if pos == -1:
        return text, None
    bigCode._datas[stock.Stock.CURRENT_PRICE][0] = text[:pos]
    text = text[pos:]
    ...
    #涨跌额。
    bigCode._datas[stock.Stock.MARKUP_VALUE][0] = dataList[1]

    #涨跌幅。
    bigCode._datas[stock.Stock.MARKUP_PERCENT][0] = dataList[2]

    #成交量。
    bigCode._datas[stock.Stock.BARGAINON_COUNT][0] = dataList[3]
...
def parseFirstStock(text):
    ...
    if data != None:
        pos = data.find("-")
        if pos != -1:
            stk._datas[i][0] = data[:pos].strip("&nbsp;")
            stk._datas[i + 1][0] = data[pos+1:].strip("&nbsp;")
